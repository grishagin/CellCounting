macro "CellCount--BrightField--Threshold--30" {
	...
	resultsFileName = "Cell counting results (Threshold-30).txt"
	...
	for (i=0; i<list.length; i++) {
		...
		if (endsWith(toUpperCase(list[i]), ".JPG")) {
			...
			run("Convert to Mask");	
			run("Analyze Particles...", "size=30-1000 circularity=0.00-1.00 show=Outlines clear include summarize");
